...
/**
 * @author yuanwanli
 * @des   对 OkHttp 的请求和响应信息进行更规范和清晰的打印, 此类为框架默认实现, 以默认格式打印信息, 若觉得默认打印格式
...
class DefaultFormatPrinter : FormatPrinter {
    ...
    private fun dotHeaders(header: String): String {
        val headers = header.split(LINE_SEPARATOR!!).dropLastWhile {
            it.isEmpty()
        }
    ...
    override fun printJsonResponse(
        ...
        bodyString =
            if (contentType.isJson()) JsonUtils.formatJson(bodyString) else if (contentType.isXml()
            ) bodyString.xmlFormat() else bodyString
        val responseBody = LINE_SEPARATOR + BODY_TAG + LINE_SEPARATOR + bodyString
        val tag = getTag(false)
        val urlLine =
            arrayOf<String?>(URL_TAG + responseUrl, N)
